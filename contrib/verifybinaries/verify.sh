...
###   blemflarkcore.org and blemflark.org and compares them.
...
WORKINGDIR="/tmp/blemflark_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://blemflarkcore.org"
HOST2="https://blemflark.org"
BASEDIR="/bin/"
VERSIONPREFIX="blemflark-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'blemflark-',
...
if [ $? -ne 0 ]; then
   echo "blemflark.org failed to provide signature file, but blemflarkcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "blemflark.org and blemflarkcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Blemflark Core binary release signing key installed?"
   fi
