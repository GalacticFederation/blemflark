...
The RPM spec file provided here is for Blemflark-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://blemflark.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Blemflark git repository](https://github.com/blemflark/blemflark) but are not part of
...
Currently only `Source10` is used, for the example `blemflark.conf` file.
...
in the blemflark git repository. At present only an SVG version of the Blemflark
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the blemflark
...
needed when 0.12.1 is released, a proper fix is already in the Blemflark git
master branch.
...
idea to build Blemflark Core against a static version of that library compiled
...
The `blemflark.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `blemflark-server` package. At this point, most RPM based
...
The `blemflark-server` package should have SELinux support. How to properly do
...
Blemflark-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/blemflark/). He has been
packaging Blemflark for Fedora at least since 2012.
...
4. I renamed the `blemflark` package that contains the Qt GUI to `blemflark-core` as
that appears to be how the general population refers to it, in contrast to
`blemflark-xt` or `blemflark-classic`. I wanted to make sure the general population
