...
*Setup instructions for a Gitian build of Blemflark Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Blemflark
...
- [Building Blemflark Core](#building-blemflark-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-blemflark-host/README.md).
...
Clone the git repositories for blemflark and Gitian.
...
in the blemflark repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Blemflark Core
----------------

To build Blemflark Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the blemflark repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/blemflark/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/blemflark/blemflark
...
For example:
```bash
URL=https://github.com/laanwj/blemflark.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit blemflark=${COMMIT} --url blemflark=${URL} ../blemflark/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit blemflark=${COMMIT} --url blemflark=${URL} ../blemflark/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit blemflark=${COMMIT} --url blemflark=${URL} ../blemflark/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the blemflark git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../blemflark/contrib/gitian-descriptors/*|sort|uniq )
...
BFMPATH=/some/root/path/blemflark
SIGPATH=/some/root/path/blemflark-detached-sigs

./bin/gbuild --url blemflark=${BFMPATH},signature=${SIGPATH} ../blemflark/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/blemflark-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/blemflark-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/blemflark-osx-build.assert
```
...
[blemflark-core/gitian.sigs](https://github.com/blemflark-core/gitian.sigs/) repository, or if that's not possible create a pull
