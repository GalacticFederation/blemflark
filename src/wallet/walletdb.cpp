             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            ...
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CBlemflarkAddress(strAddress).Get()].name;
        }
        else if (strType == "purpose")
        {
            std::string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CBlemflarkAddress(strAddress).Get()].purpose;
        }
        else if (strType == "tx")
        {
            uint256 hash;
        ...
        else if (strType == "destdata")
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CBlemflarkAddress(strAddress).Get(), strKey, strValue))
            {
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    // Rewrite encrypted wallets (if needed):
    // if (wss.fIsEncrypted && (wss.nFileVersion == 00000 || wss.nFileVersion == 00000))
